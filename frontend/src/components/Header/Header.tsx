...
function Header({ocult}: HeaderProps) {
    ...
    return (
        <header className="w-full bg-white">
            <div className="container mx-auto flex items-center justify-evenly py-4">
                ...
                <button 
                    className="lg:hidden p-2 text-gray-500 hover:text-gray-700 focus:outline-none z-40" 
                    onClick={toggleMenu} 
                ...
                <nav 
                    className={`w-auto lg:flex lg:items-center lg:space-x-4 lg:justify-evenly lg:w-full lg:h-auto lg:bg-transparent 
                        ${isMenuOpen ? 'absolute w-screen top-16 bg-white shadow-lg' : 'hidden lg:flex'}`}
                    style={{ zIndex: 40 }}
                >
                    <div className={`flex w-full flex-col items-center justify-center lg:flex-row lg:space-x-4 p-4 lg:p-0`}>
                        <HeaderGeneralLinks />
                        <HeaderAccountLinks ocult={ocult}/>
                    </div>
                </nav>
            </div>
        </header>
