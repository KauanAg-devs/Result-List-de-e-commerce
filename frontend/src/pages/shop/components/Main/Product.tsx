...
const Product: React.FC<ProductType> = ({ image, name, title, price, discount }) => {
  ...
  return (
    <div 
      className="relative flex flex-col items-center bg-[#F4F5F7] w-full sm:w-[200px] md:w-[250px] cursor-pointer mb-10"
      ...
      {showProductDetails && (
        ...
        >
          ...
          <div className="flex justify-evenly items-center w-full mt-2">
            <img className="w-[12%] sm:w-[14%] md:w-[16%]" src={ShareImage} alt="Share" />
            <img className="w-[12%] sm:w-[14%] md:w-[16%]" src={CompareImage} alt="Compare" />
            <img className="w-[12%] sm:w-[14%] md:w-[16%]" src={LikeImage} alt="Like" />
          </div>
        </div>
      )}
      <img className="w-full h-[70%]-cover" src={image} alt={name} />
      {discountPercentage && (
        <div className={`absolute right-1 top-1 flex items-center justify-center text-center h-12 w-12 ${discountPercentage > 0 ? 'bg-[#2ec1ac]' : 'bg-[#e97171]'} rounded-full`}>
          {discountPercentage}%
        </div>
      )}
      <div className="flex-col items-center mt-2 h-[15%] w-full px-2">
        <div className="text-xs sm:text-sm md:text-base font-bold text-[#3A3A3A] truncate">{name}</div>
        <div className="text-xs sm:text-sm md:text-sm text-[#898989] font-[550] mt-1 truncate">{title}</div>
